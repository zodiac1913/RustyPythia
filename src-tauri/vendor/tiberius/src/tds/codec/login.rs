...
impl<'a> LoginMessage<'a> {
    ...
    pub fn change_password(&mut self, password: impl Into<Cow<'a, str>>) {
        self.option_flags_3
            .insert(OptionFlag3::RequestChangePassword);
        self.change_password = password.into();
    }
...
impl<'a> Encode<BytesMut> for LoginMessage<'a> {
    fn encode(self, dst: &mut BytesMut) -> crate::Result<()> {
        ...
        let var_data = [
            ...
            &"".into(), // 10. ibSSPI
            &"".into(), // ibAtchDBFile
            &self.change_password, // ibChangePassword
        ];
...
mod tests {
            ...
            Self: Sized,
        {
            ...
            macro_rules! read_offset_length_string {
                ...
                ($tag:expr) => {{
                    ...
                    if $tag == "password" || $tag == "change_password" {
                        let buffer = cursor.get_mut();
                        for byte in buffer
                            .iter_mut()
            ...
            ret.change_password = read_offset_length_string!("change_password").into();
            // let _ = cursor.read_u32::<LittleEndian>()?;
            // cbSSPILong
    ...
    fn login_message_with_password_change_round_trip() {
        ...
        login.change_password("new-password");
        login
            .clone()
            .encode(&mut payload)
            .expect("encode should succeed");
